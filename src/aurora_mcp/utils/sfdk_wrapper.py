...
class SFDKOutputFilter:
    ...
    def __init__(self):
        ...
        self.show_compiled = [re.compile(pattern, re.IGNORECASE) for pattern in self.show_patterns]
        self.hide_compiled = [re.compile(pattern, re.IGNORECASE) for pattern in self.hide_patterns]
    ...
    def should_show_line(self, line: str) -> bool:
        ...
        if any(keyword in line.upper() for keyword in ['ERROR', 'FAILED', 'FATAL', 'CRITICAL']):
            return True
    ...
    def get_progress_from_line(self, line: str) -> Optional[tuple[float, str]]:
        ...
        if percentage_match:
            ...
            mapped_percent = 30 + (percent * 0.5)  # Scale cmake 0-100% to our 30-80% range
        ...
        elif "CMake" in line and ("build" in line.lower() or "compiling" in line.lower()):
...
def truncate_output(output: str, max_lines: int = 50, max_chars: int = 5000) -> str:
    ...
    truncated_lines = lines[:half_lines] + ['...', f'[Output truncated - {len(lines)} total lines]', '...'] + lines[-half_lines:]
    
    truncated = '\n'.join(truncated_lines)
    
    # If still too long, truncate by characters
    if len(truncated) > max_chars:
        truncated = truncated[:max_chars-50] + '...\n[Output truncated for response size]'
...
class SFDKWrapper:
    ...
    async def get_target_for_arch(self, target_arch: str) -> Optional[str]:
        ...
        if not targets_result["success"]:
            logger.warning(f"Could not list SFDK targets: {targets_result.get('error')}")
            return None
    ...
    async def build_project(
        ...
        try:
            ...
            if context:
                ...
                await context.report_progress(12, 100, f"Target selected: {target_name}")
            ...
            if not config_result["success"]:
                logger.warning(f"Could not configure SFDK target: {config_result.get('output')}")
                if context:
                    await context.warning("⚠️ Target configuration had issues, but continuing...")
            else:
            ...
            build_result = await self.create_build_directory(project_path, build_dir_name)
            if not build_result["success"]:
                error_msg = f"Failed to create build directory: {build_result.get('error')}"
                if context:
            ...
            logger.info(f"Executing SFDK build command from {build_path}: {' '.join(build_cmd)}")
            ...
            if build_exec_result["success"]:
                ...
                if context:
                    if artifacts:
                        ...
                        await context.warning("⚠️ No RPM artifacts found - build may not have created packages")
                ...
                return {
                    ...
                    "output_truncated": len(full_output) > 5000 or len(full_output.split('\n')) > 50
                }
            else:
    ...
    async def create_build_directory(self, project_path: Path, build_dir_name: str = "build_amogus") -> Dict[str, Any]:
