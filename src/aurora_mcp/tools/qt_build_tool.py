...
class QtBuildTool:
    ...
    async def build_project(
        ...
        try:
            ...
            if not project_dir.is_absolute():
                error_msg = f"Project path must be absolute (start with /). Got: {project_path}"
            ...
            if tool_type == "psdk":
                ...
                result = await self._build_with_sfdk(
                    project_dir, project_type, build_type, target_arch, build_dir_name, context
                )
            else:
    ...
    async def _select_build_tool(self, build_tool: Optional[str] = None) -> Dict[str, Any]:
        ...
        if selected_tool == "sfdk":
            if await self.sfdk.is_available():
                ...
                return {
                    "success": False,
                    "error": "SFDK requested but not available"
                }
            ...
            if self.psdk_path.exists():
                ...
                return {
                    "success": False,
                    "error": "PSDK requested but not found"
                }
    ...
    async def _build_with_psdk(
        ...
        try:
            ...
            if result.get("success"):
                ...
                logger.error(f"PSDK build failed: {result.get('error', 'Unknown error')}")
                if context:
                    await context.error(f"❌ PSDK build failed: {result.get('error', 'Unknown error')}")
        ...
        for i, cmd in enumerate(commands):
            ...
            if proc.returncode != 0:
                ...
                return {
                    ...
                    "output_truncated": len(output) > 5000 or len(output.split('\n')) > 50
                }
    ...
    async def _build_with_sfdk(
        ...
        try:
            ...
            result = await self.sfdk.build_project(project_dir, target_arch, build_dir_name, context)
            ...
            if result["success"]:
                ...
                logger.error(f"SFDK build failed: {result.get('error', 'Unknown error')}")
    ...
    async def configure_environment(self, target_arch: str = "armv7hl") -> Dict[str, Any]:
        ...
        try:
            ...
            if sfdk_available:
                ...
                if targets_result["success"]:
                    ...
                    logger.warning(f"Could not get SFDK targets: {targets_result.get('error')}")
                    sfdk_info["targets"] = []
            ...
            if psdk_available:
                ...
                if targets_dir.exists():
                    psdk_targets = [
                        t.name for t in targets_dir.iterdir() 
