...
async def create_qt_project(
    ctx: Context,
    workspace_dir: str,
    type: Literal["cmake", "qmake"],
    organization_name: str = "ru.auroraos",
    application_name: str = "ApplicationTemplate",
) -> dict[str, Any]:
    """Create Qt project for Aurora OS from GitLab template.

    This tool clones the ApplicationTemplate repository from omprussia GitLab
    ...
    Args:
        ctx: FastMCP context
        type: Project type - must be "cmake" or "qmake"
    ...
    try:
        ...
        repo_url = "https://gitlab.com/omprussia/demos/ApplicationTemplate"
        branch = "example" if type == "qmake" else "cmake-version"
        ...
        return {
            "status": "success",
            "message": f"Successfully created Qt {type} project '{application_name}' in workspace directory",
            "project_type": type,
            "organization_name": organization_name,
