...
class RPMPackagingTool:
    ...
    async def _create_source_tarball(self, source_dir: Path) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "tarball": str(tarball_path)}
    ...
    async def _build_rpm(
        ...
        try:
            ...
            if proc.returncode != 0:
                return {"success": False, "error": "RPM build failed", "output": output}
    ...
    async def sign_package(
        ...
        try:
            ...
            if not rpm_path.exists():
                return {"success": False, "error": f"RPM file not found: {rpm_file}"}
    ...
    async def _check_dependencies(self, rpm_file: Path) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "requires": deps, "dependency_count": len(deps)}
